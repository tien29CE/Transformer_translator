...
def greedy_decode(model, source, source_mask, tokenizer_src, tokenizer_tgt, max_len, device):
    ...
    while True:
        ...
        _, next_word = torch.max(torch.softmax(logits, dim= -1), dim=-1)
...
def get_model(config, vocab_src_len, vocab_target_size):
    model = build_transformer(vocab_src_len, vocab_target_size, config['seq_len'], config['seq_len'], config['d_model'])
    return model
...
def train_model(config: dict):
    ...
    loss_function = nn.CrossEntropyLoss(ignore_index=tokenizer_target.token_to_id('[PAD]'), label_smoothing=0.1).to(device)
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            # Run the tensor thourgh the transformer
